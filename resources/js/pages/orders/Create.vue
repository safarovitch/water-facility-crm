  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="relative overflow-x-auto sm:rounded-lg">
      <div class="pb-6 bg-white dark:bg-gray-900 px-4 py-5 sm:px-6 rounded-t-lg">
        <h1 class="text-xl font-semibold text-gray-900 dark:text-white">{{ t('Create New Order') }}</h1>
      </div>
      ...
      <form @submit.prevent="submitForm" class="space-y-6">
          ...
          <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div class="grid gap-2 sm:col-span-2">
              <div class="flex items-center justify-between">
                ...
                >
                  {{ isNewContact ? t('← Pick existing client') : t('+ New walk-in client') }}
                </button>
              </div>
              ...
              >
                <p class="sm:col-span-2 text-xs text-sky-800 dark:text-sky-200">
                  {{ t('A client profile will be created from these details. If they later register with the same phone or email, the account will be adopted.') }}
                </p>
                <div class="grid gap-1">
                  <Label class="text-xs">{{ t('Name') }} *</Label>
                  <Input v-model="form.new_contact!.name" :placeholder="t('Client name')" />
                  <InputError :message="(form.errors as any)['new_contact.name']" />
                </div>
                <div class="grid gap-1">
                  <Label class="text-xs">{{ t('Phone') }} *</Label>
                  <Input v-model="form.new_contact!.phone" placeholder="+992 …" />
                  <InputError :message="(form.errors as any)['new_contact.phone']" />
                </div>
                <div class="grid gap-1 sm:col-span-2">
                  <Label class="text-xs">{{ t('Email') }} <span class="text-gray-400 font-normal">({{ t('optional') }})</span></Label>
            ...
            <div class="grid gap-2 sm:col-span-2">
              ...
              <div v-if="!isAddingNewAddress" class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-2">
                ...
                <div
                  ...
                  <div>
                    <div class="font-medium text-gray-900 dark:text-white">{{ t('Self Pickup') }}</div>
                    <div class="text-xs text-gray-500 mt-1">{{ t('Client will collect the order — no delivery needed.') }}</div>
                  </div>
                </div>
              ...
              <div v-if="!isAddingNewAddress" class="flex justify-start">
                <Button type="button" variant="link" size="sm" @click="toggleNewAddress" class="px-0 h-auto">
                  {{ t('+ Add new address for this client') }}
                </Button>
              </div>
              ...
              <div v-if="isAddingNewAddress" class="space-y-3 p-4 rounded-xl border border-blue-100 bg-blue-50/30 dark:border-blue-900/30 dark:bg-blue-900/10">
                <div class="flex items-center justify-between">
                  <h3 class="text-sm font-medium text-blue-900 dark:text-blue-100">{{ t('Adding New Address') }}</h3>
                  <Button type="button" variant="ghost" size="sm" @click="toggleNewAddress">{{ t('Cancel') }}</Button>
                </div>
                <div class="grid gap-3">
                  <div class="grid gap-1.5">
                    <Label for="new_address_label" class="text-xs">{{ t('Address Label (e.g. Home, Office)') }}</Label>
                    <Input id="new_address_label" v-model="form.new_address_label" :placeholder="t('Office')" />
                  </div>
                  <div class="grid gap-1.5">
                    <Label for="new_address" class="text-xs">{{ t('Address Line') }} *</Label>
                    <textarea id="new_address" v-model="form.new_address" rows="2" class="block w-full rounded-md border border-gray-300 bg-transparent px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white" :placeholder="t('Street, building...')"></textarea>
                  </div>
                </div>
              </div>
              ...
              <div v-if="!isAddingNewAddress && clientAddresses.length === 0 && !isSelfPickup" class="mt-2 p-4 text-center border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
                <p class="text-xs text-gray-500 mb-2">{{ t('No addresses found for this client.') }}</p>
        ...
        <div class="flex items-center justify-end space-x-3 pt-2 pb-6">
          ...
          >
            ...
            <span v-else>{{ t('Create Order') }}</span>
          </Button>
        </div>
      </form>
    </div>
