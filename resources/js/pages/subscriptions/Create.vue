  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="relative overflow-x-auto sm:rounded-lg">
      <div class="pb-6 bg-white dark:bg-gray-900 px-4 py-5 sm:px-6 rounded-t-lg">
        <h1 class="text-xl font-semibold text-gray-900 dark:text-white">Create Recurring Subscription</h1>
        <p class="text-sm text-muted-foreground mt-1">Set up automatic recurring water deliveries for a client.</p>
      </div>
      ...
      <form @submit.prevent="submitForm" class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg px-4 py-5 sm:p-6">
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div class="grid gap-2 sm:col-span-2">
              ...
              <textarea v-model="form.delivery_address" rows="2" placeholder="Enter delivery address..." class="block w-full rounded-md border border-gray-300 bg-transparent px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white"></textarea>
        ...
        <!-- Schedule -->
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg px-4 py-5 sm:p-6">
          <h2 class="text-base font-semibold text-gray-900 dark:text-white mb-4">Schedule</h2>
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="grid gap-2">
              ...
              <select v-model="form.frequency" class="border-input flex h-10 w-full rounded-md border bg-transparent px-3 py-2 text-sm dark:bg-input/30 dark:border-gray-600 dark:text-white">
                <option v-for="f in props.frequencies" :key="f.value" :value="f.value">{{ f.label }}</option>
              </select>
            ...
            <div v-if="showDayOfMonth" class="grid gap-2">
              ...
              <Input type="number" min="1" max="31" v-model.number="form.day_of_month" placeholder="e.g. 15" class="h-10" />
            </div>
            ...
            <div class="grid gap-2">
              ...
              <select v-model="form.time_slot" class="border-input flex h-10 w-full rounded-md border bg-transparent px-3 py-2 text-sm dark:bg-input/30 dark:border-gray-600 dark:text-white">
                ...
                <option v-for="ts in props.timeSlots" :key="ts.value" :value="ts.value">{{ ts.label }}</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg px-4 py-5 sm:p-6">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-base font-semibold text-gray-900 dark:text-white">Products</h2>
            <Button type="button" variant="outline" @click="addItem">+ Add Product</Button>
          </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg px-4 py-5 sm:p-6">
          <div class="grid gap-2">
            ...
            <textarea id="notes" v-model="form.notes" rows="2" class="block w-full rounded-md border border-gray-300 bg-transparent px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white" placeholder="Optional delivery instructions..."></textarea>
          </div>
        </div>

        <div class="flex items-center justify-end space-x-3 pt-2 pb-6">
          <Button type="button" @click="$inertia.visit('/admin/subscriptions')" variant="outline">Cancel</Button>
