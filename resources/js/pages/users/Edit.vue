...
<template>
  ...
  <Head :title="t('Editing: {name}', { name: user.name })" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="relative overflow-x-auto sm:rounded-lg">
      ...
      <div class="p-4 flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-6 bg-white dark:bg-gray-900">
        <div>
          <h1 class="text-xl font-semibold text-gray-900 dark:text-white">{{ t('Editing {name}', { name: user.name }) }}</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ t('Update user information') }}</p>
        </div>
      </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg">
          <div class="px-4 py-5 sm:p-6">
            ...
            <div class="grid gap-2 mb-6">
              <Label for="name">{{ t('Avatar') }}</Label>
              <div class="flex items-center space-x-4">
                <div class="relative">
                  <div class="w-20 h-20 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center overflow-hidden">
                    <img v-if="avatarPreview" :src="avatarPreview" :alt="t('Avatar preview')" class="w-full h-full object-cover">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div class="grid gap-2">
                ...
                <Input id="name" class="mt-1 block w-full" name="name" v-model="form.name" required autocomplete="name" :placeholder="t('Full name')" />
              ...
              <div class="grid gap-2">
                ...
                <Input id="email" class="mt-1 block w-full" name="email" v-model="form.email" autocomplete="email" :placeholder="t('Email Address')" />
              ...
              <div class="grid gap-2">
                ...
                <Input id="phone" class="mt-1 block w-full" name="phone" v-model="form.phone" autocomplete="tel" :placeholder="t('Phone Number')" />
              ...
              <div class="grid gap-2">
                ...
                <select id="status" name="status" v-model="form.status" required :class="cn(
                  ...
                  <option v-for="(statusIndex, status) in statuses" :key="statusIndex" :value="statusIndex">{{ t(String(status)) }}</option>
                </select>
              </div>

              <!-- Roles -->
              <div class="grid gap-2">
                <Label for="roles">{{ t('Assign roles') }}</Label>
            ...
            <div class="mt-8 mb-4">
              <h3 class="text-md font-medium text-gray-900 dark:text-white pb-2 border-b border-gray-200 dark:border-gray-700">{{ t('Asterisk SIP Credentials') }}</h3>
            </div>
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 mt-5">
              ...
              <div class="grid gap-2">
                ...
                <Input id="sip_password" type="text" class="mt-1 block w-full" name="sip_password" v-model="form.sip_password" :placeholder="t('SIP Password')" />
            ...
            <!-- Login Credentials -->
            <div class="mt-8 mb-4">
              <h3 class="text-md font-medium text-gray-900 dark:text-white pb-2 border-b border-gray-200 dark:border-gray-700">{{ t('Login Credentials') }}</h3>
            </div>
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 mt-5">
              <!-- Password -->
              <div class="grid gap-2">
                <Label for="password">{{ t('Password') }} *</Label>
                <Input id="password" class="mt-1 block w-full" name="password" v-model="form.password" autocomplete="new-password" :placeholder="t('Enter password (min. 8 characters)')" />
