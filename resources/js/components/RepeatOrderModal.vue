    ...
    <DialogContent class="sm:max-w-md">
      <DialogHeader>
        ...
        <DialogDescription v-if="orderNumber">
          New order based on <span class="font-mono">#{{ orderNumber }}</span>. Adjust the quantities and delivery time.
        </DialogDescription>
        <DialogDescription v-else>
          Choose the quantities and a delivery time, then place the order.
        </DialogDescription>
      </DialogHeader>
        ...
        <ul class="rounded-xl border border-slate-200 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-700">
          <li v-for="line in lines" :key="line.product_id" class="flex items-center gap-3 px-3 py-2.5">
            <div class="flex-1 min-w-0">
              ...
              <span v-if="line.is_gift" class="text-[10px] font-bold uppercase tracking-wide text-pink-600">Gift</span>
            </div>
          ...
          <li v-if="!lines.length" class="px-3 py-3 text-xs italic text-slate-500">No items to repeat.</li>
        </ul>
        <p class="text-right text-xs text-slate-500">
          Total bottles: <span class="font-semibold text-slate-700 dark:text-slate-200">{{ totalBottles }}</span>
        </p>

        <!-- Delivery time -->
        <div>
          <label class="text-[11px] uppercase tracking-wider text-slate-400">Delivery time</label>
          <DeliveryTimePicker v-model="scheduledAt" auto-default class="mt-1" />
          <p class="mt-1 text-[11px] text-slate-400">Slots run 09:00–20:00 in half-hour steps.</p>
        </div>
      </div>
